...
A momentary button GP14→GND (internal pull-up, active-LOW) fires a falling-edge GPIO ISR (`estop_isr`) that **cuts the TMC output stage in hardware** (`gpio_put(PIN_ENN, !ENN_ON_LEVEL)`) right there — no SPI, no motion task, ~µs — independent of everything. It then latches `tmc.hard_off` (so `tmc5072_enable()` is forced off and the motion task's per-move `enable(true)` calls become no-ops) and sets `g_estop` (so `motion_should_abort()` bails the in-flight move). **The motion math/streaming is untouched** — the only change in the driver is the `hard_off` check in `tmc5072_enable()`. `g_estop` is reported in `/api/status`; recover with `plotter_clear_fault()` (console `estopclr`, `/api/clearfault`, `plot_clear_fault`, or the console's ⛔ banner) which releases the latch + re-enables — **re-home after**, since motor power was physically cut.
