...
var textPathModule = {
  ...
  sections: [
    ...
    { title: "Path", fields: [
      ...
      { key: "length", label: "Path length", type: "range", min: 50, max: 2e3, step: 10, unit: "mm", default: 500 },
      { key: "wander", label: "Walk wander", type: "range", min: 0, max: 3, step: 0.05, default: 0.8 },
      { key: "headingDeg", label: "Start heading", type: "range", min: -180, max: 180, step: 5, unit: "\xB0", default: 0 },
  ...
  generate(params) {
    ...
    const font = strokeFontDriver(params.font === "bold" ? "bold" : "sans");
    ...
    const spine = [];
    if (spineKind === "circle") {
      const r2 = Math.max(5, num(params, "circleR", 80));
      const n = Math.max(16, Math.ceil(2 * Math.PI * r2 / STEP));
      const a0 = num(params, "headingDeg", 0) * Math.PI / 180 - Math.PI / 2;
      for (let i = 0; i <= n; i++) {
        const a = a0 + 2 * Math.PI * i / n;
        spine.push({ x: x0 + r2 * Math.cos(a), y: y0 + r2 * Math.sin(a) });
      }
    ...
    const at = (s2) => {
      const f = Math.max(0, Math.min(spine.length - 2, s2 / STEP));
    ...
    const paths = [];
    const baselineOff = params.baseline === "above" ? -size : -size / 2;
    let s = 0;
    let idx = 0;
    const sep = repeat ? " " : "";
    for (let guard = 0; guard < 4e3; guard++) {
      ...
      for (const stroke2 of font.renderRun(ch, size, 0)) {
        if (stroke2.length < 2) continue;
        const pts = stroke2.map((g) => {
